...
img_diff = dict()
state_dict = torch.load('checkpoints/PAN/PAN_PA_something_resnet50_shift8_blockres_avg_segment8_e80.pth.tar')
for k, v in state_dict.items():
    conv_weight = v['module.PA.shallow_conv.weight']
    conv_bias = v['module.PA.shallow_conv.bias']

# x = torch.rand(1,87,8,72000)
# x_1 = torch.rand(8,72000,1)
# x_2 = torch.rand(8,72000,1)
# d_i = nn.PairwiseDistance(p=2)(x_1, x_2)
# print(d_i.size())
# exit()
...
class PA(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x original size: ", x.size())
        x = x.view(-1, self.n_length, x.size(-3), x.size(-2)*x.size(-1))
        # print("x.size(): ", x.size()) # torch.Size([1, 87, 8, 72000])
        x = x.unsqueeze(-1)
        # print("x.size(): ", x.size()) # torch.Size([1, 87, 8, 72000, 1])
        for i in range(self.n_length-1):
            d_i = nn.PairwiseDistance(p=2)(x[:,i,:,:,:], x[:,i+1,:,:,:]).unsqueeze(1)
            # d_i = nn.PairwiseDistance(p=3)(x[:,i,:,:], x[:,i+1,:,:])
            # print("d_i: ", d_i.size())
...
video_list = read_lines(ann_file_train)[:2000]
for item in video_list:
   ...
   # video_length =67
    ...
    tmpl = 'img_{:05}.jpg'
    pic1 = path + "img_00001.jpg"
    tmp = cv2.imread(pic1)
    tmp = tmp.transpose(2, 0, 1)
    tmp = torch.from_numpy(tmp)
    tmp = tmp.unsqueeze(0) # 1, 3, 240, 300
    # print("tmp: ", tmp.size())
    ...
    PA_module = PA(n_length=video_length).to(device)  # adjacent '4' frames are sampled for computing PA
    # PA_module = PA(n_length=4).to(device)  # adjacent '4' frames are sampled for computing PA
    ...
    # print("PA_out: ", PA_out.size())
    PA_out = PA_out.squeeze(0)  # [42,100,180]
    motion = list()
    for i in range(video_length-1):
        full_image = torch.zeros(PA_out[0, i, :, :].size()).to(device)
        for t in range(PA_out.size(0)):
            img = PA_out[t, i, :, :]
            plt.imshow(img.cpu().detach().numpy(), cmap='gray')
            # plt.imsave('image_diff/'+ str(i) +tmpl.format(t),img.cpu().detach().numpy(),cmap='gray')
            full_image += torch.sum(img)
        motion.append((torch.sum(full_image)).item())
        # TODO: solve this extra dimension issue!!
        # [8, 86, 240, 300] -> [i, 240, 300]
...
fileObject = open('img_diff.json', 'a+')
